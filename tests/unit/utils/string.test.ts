...
describe('stringInject', () => {
  context('replace brackets with array items', () => {
    it('replaces brackets {0} in string with array[0]', () => {
      ...
      expect(str).to.eq('My username is tjcafferkey');
    });
    ...
    it('replaces brackets {0} and {1} in string with array[0] and array[1]', () => {
      ...
      expect(str).to.eq('I am testing the stringInject function');
    });
  });
  ...
  context('pass in a string with no {} with an array of items', () => {
    it('returns the same string as passed in', () => {
      ...
      expect(str).to.eq('This should be the same');
    });
  });
  ...
  context('replace object values based on their keys', () => {
    ...
    it('replaces object values based on two keys', () => {
      ...
      expect(str).to.eq('My username is tjcafferkey on GitHub');
    });
    ...
    it('replaces object values based on two keys in reverse order', () => {
      ...
      expect(str).to.eq('My username is GitHub on tjcafferkey');
    });
    ...
    it('does not replace in the string if the key does not exist in the object', () => {
      ...
      expect(str).to.eq('My username is {platform} on tjcafferkey');
    });
    ...
    it('replaces object values based on one nested key and one regular', function () {
      ...
      expect(str).to.eq('My username is Robert on IRL');
    });

    it('returns the string if the object has no keys', () => {
      const str = stringInject('My username is {platform} on {username}', {});
      expect(str).to.eq('My username is {platform} on {username}');
    });
  });
